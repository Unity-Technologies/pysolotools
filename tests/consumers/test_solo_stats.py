...
from pysolotools.consumers.solo_stats import (
    BBOX_HEATMAP_NAME,
    STATS_PATH_NAME,
    TOTAL_TOKEN,
    SoloStats,
...
class TestConsumerSoloStats:
    ...
    def test_get_object_count_by_id(self, mock_get_object_count, setup):
        ...
        assert (
            ...
            == mock_get_object_count.return_value
        )
        mock_get_object_count.assert_called_once_with(["Cube", "Crate"])

    @patch.object(SoloStats, "get_bbox_per_img_dist_by_labels")
    def test_get_bbox_per_img_dist_by_ids(
        self, mock_get_bbox_per_img_dist_by_ids, setup
    ):
        solo_stats, _ = setup
        assert (
            solo_stats.get_bbox_per_img_dist_by_ids(category_ids=[1, 2])
            == mock_get_bbox_per_img_dist_by_ids.return_value
        )
    ...
    def test_get_bbox_per_img_dist_by_labels(self, setup, test_input, expected):
        solo_stats, _ = setup
        assert (
            solo_stats.get_bbox_per_img_dist_by_labels(category_ids=test_input)
            == expected
        )
    ...
    def test_get_bbox_heatmap_by_labels(self, setup, test_input, expected):
        ...
        assert_array_equal(
            x=solo_stats.get_bbox_heatmap_by_labels(category_ids=test_input), y=expected
        )
    ...
    def test_get_bbox_size_dist_by_labels(self, setup, test_input, expected):
        solo_stats, _ = setup
        assert (
            solo_stats.get_bbox_size_dist_by_labels(category_labels=test_input)
            == expected
        )
