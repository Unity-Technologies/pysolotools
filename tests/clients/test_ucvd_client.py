...
from unittest.mock import patch, call, Mock, create_autospec, MagicMock
...
from pysolotools.core import UCVDDataset, UCVDArchive, UCVDAttachment
...
@pytest.fixture
@patch('pysolotools.clients.ucvd_client.UCVDClient._UCVDClient__make_request', autospec=True)
...
@pytest.mark.parametrize(
    ...
    [
        (None, None, {'method': 'post', 'url': 'some-uri/organizations/mock-org-id/projects/mock-proj-id/datasets',
                      'body': {"name": 'some-dataset'}}),
        (None, 'a-license',
         {'method': 'post', 'url': 'some-uri/organizations/mock-org-id/projects/mock-proj-id/datasets',
...
def test_create_dataset(setup_client: TestFixture, description, license_uri, expected):
    assert setup_client.client.create_dataset(
        dataset_name='some-dataset',
        description=description,
        license_uri=license_uri
    ) == mock_make_request_response
    setup_client.mock_make_request.assert_called_once_with(**expected, auth=setup_client.client.auth)


def test_describe_dataset(setup_client: TestFixture):
    assert setup_client.client.describe_dataset(dataset_id='some-dataset') == mock_make_request_response
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'dataset_id': 'some-dataset', 'archive_name': 'some-archive'}, 'some-archive'),
    ]
)
def test_create_dataset_archives(setup_client: TestFixture, arguments, expected_archive_name):
    assert setup_client.client.create_dataset_archive(**arguments) == mock_make_request_response
    setup_client.mock_make_request.assert_called_once_with(
        method='post',
...
def test_list_dataset_archive(setup_client: TestFixture):
    assert setup_client.client.list_dataset_archives(dataset_id='some-dataset') == 'the results'
    setup_client.mock_make_request.assert_called_once_with(
        method='get',
        url='some-uri/organizations/mock-org-id/projects/mock-proj-id/datasets/some-dataset/archives',
        auth=setup_client.client.auth
...
def test_describe_dataset_archive(setup_client: TestFixture):
    assert setup_client.client.describe_dataset_archive(
        dataset_id='some-dataset',
        archive_id='some-archive'
    ) == mock_make_request_response
    setup_client.mock_make_request.assert_called_once_with(
        method='get',
...
def test_list_dataset_attachments(setup_client: TestFixture):
    assert setup_client.client.list_dataset_attachments(dataset_id='some-dataset') == 'the results'
    setup_client.mock_make_request.assert_called_once_with(
        method='get',
...
def test_describe_dataset_attachment(setup_client: TestFixture):
    assert setup_client.client.describe_dataset_attachment(
        dataset_id='some-dataset',
...
def test_create_dataset_attachment(setup_client: TestFixture):
    assert setup_client.client.create_dataset_attachment(
        dataset_id='some-dataset',
...
def test_create_build(setup_client: TestFixture):
    assert setup_client.client.create_build(
...
def test_describe_build(setup_client: TestFixture):
    assert setup_client.client.describe_build(build_id='some-build') == mock_make_request_response
    setup_client.mock_make_request.assert_called_once_with(
        method='get',
...
@patch('pysolotools.clients.ucvd_client.UCVDClient._UCVDClient__iterable_get', autospec=True)
...
def test_iterate_dataset_archives(mock_iterable_get, setup_client: TestFixture):
    ...
    setup_client.client._UCVDClient__iterable_get = mock_iterable_get
    for result in setup_client.client.iterate_dataset_archives(dataset_id='some-dataset'):
...
def test_iterate_dataset_attachments(mock_iterable_get, setup_client: TestFixture):
    ...
    setup_client.client._UCVDClient__iterable_get = mock_iterable_get
    for result in setup_client.client.iterate_dataset_attachments(dataset_id='some-dataset'):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'solo_dir': 'some-dir', 'subdir': True}, False, 'some-dir/solo', [call('some-dir/solo')]),
    ]
)
...
def test_extract_dataset(
        mock_makedirs, mock_exists, mock_glob, mock_open, setup_client: TestFixture,
        arguments, exists, expected_root_path, expected_makedirs
):
...
def test_download_dataset_archives(
        mock_isdir, mock_mkdir, mock_iterate_dataset_archives, mock_open, setup_client: TestFixture,
        is_dir, expected_makedirs
):
    ...
    mock_iterate_dataset_archives.assert_called_once_with(setup_client.client, 'some-dataset')
...
def test_download_dataset_attachment(mock_describe_dataset_attachment, mock_open, setup_client: TestFixture):
    mock_describe_dataset_attachment.return_value = UCVDAttachment(**mock_dataset_attachment)
    mock_write = MagicMock()
    mock_open.return_value.__enter__.return_value.write = mock_write
    responses.add(
        responses.GET,
    ...
    mock_open.assert_called_once_with('some-dir/Sample.tar', 'wb')
    mock_describe_dataset_attachment.assert_called_once_with(setup_client.client, 'some-dataset', 'some-attachment')
    mock_write.assert_called_once_with(b'stuff')
...
@patch('builtins.open', autospec=True)
def test_upload_file(mock_open, mock_getsize, mock_put, mock_streaming, setup_client: TestFixture):
    ...
    assert setup_client.client.upload_file(
        url='some-url',
...
@pytest.mark.parametrize(
    ...
    [
        (200, {'next': '1234', 'results': ['some stuff']}, ['some stuff', 'next stuff'], does_not_raise()),
        (200, {'results': ['some stuff']}, ['some stuff'], does_not_raise()),
        (200, {'results': ['some stuff', 'more stuff']}, ['some stuff', 'more stuff'], does_not_raise()),
        (404, {}, [], pytest.raises(HTTPError))
    ]
)
def test___iterable_get(setup_client: TestFixture, status_code, json_response, expected, expectation):
    responses.add(
        responses.GET,
        url='http://some-url',
        json=json_response,
    ...
    responses.add(
        responses.GET,
        url='http://some-url?next=1234',
        json={'results': ['next stuff']}
    )
    items = []
    with expectation:
        for response in setup_client.client._UCVDClient__iterable_get(base_url='http://some-url'):
...
@pytest.mark.parametrize(
    ...
    [
        ('good', does_not_raise(), 'some headers', 'some auth', 'some params', 'some body', 'some data'),
